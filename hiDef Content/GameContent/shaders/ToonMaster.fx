...
float3			RotationDirection	= float3(0, 1, 0);

float			Time				: TIME;

float4			ModelColor;						//per model

float4x4		FinalTransforms		[56];		//bones
...
texture BillboardTexture : register(t8);
sampler billboardSampler : register(s8) = sampler_state
{
...
// toon controllers
//-----------------
float	ToonThresholds[2]				= { 0.6, 0.2 };
...
AnimatedMesh_VS_OUTPUT AnimatedMesh_vs_main( float4 inPosition		: POSITION0,
												float3 inNormal		: NORMAL0,
												float2 inTexCoord	: TEXCOORD0,
												float4 inWeights	: BLENDWEIGHT0,
												int4  inBoneIndices : BLENDINDICES0 )
{
	AnimatedMesh_VS_OUTPUT output;
	...
	output.TexCoord = inTexCoord;
	float3 worldNormal = normalize(mul(normal, World));
	output.LightAmount = dot(worldNormal, LightDirection);
...
//==============================================================
//					ANIMATED MESH - DEPTH & NORMAL
//==============================================================

NormalDepth_VS_OUTPUT AnimatedMeshDepth_vs_main( float4 inPosition : POSITION0,
													float3 inNormal : NORMAL0,
													float2 inTexcoord : TEXCOORD0,
													float4 inWeights	: BLENDWEIGHT0,
													int4  inBoneIndices : BLENDINDICES0 )
...
{
	line_VS_OUTPUT output = (line_VS_OUTPUT)0;
...
// 3d line drawing technique					//3D LINE
technique Line3D
{
	pass P0
	{
...
// toon-lit animated mesh						//ANIMATED MESH
technique AnimatedMesh
{
	pass P0
	{
		VertexShader = compile vs_2_0 AnimatedMesh_vs_main();
		PixelShader = compile ps_2_0 ToonPixelShader();
	}
}

// depth&normal rendering for animated meshes	//ANIMATED MESH DEPTH
technique AnimatedMeshDepth
{
	Pass P0
	{
		VertexShader = compile vs_2_0 AnimatedMeshDepth_vs_main();
		PixelShader = compile ps_2_0 NormalDepthPixelShader();
	}
}